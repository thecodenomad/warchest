...
func TestConfig(t *testing.T) {
	...
	t.Run("Test Valid Config", func(t *testing.T) {
		...
		testConfigFile := LocalConfigFile{Filepath: "./testdata/CoinConfig.json"}
		...
		valueTests := []struct {
			...
			{tmpConfig.Transactions[1].PurchasedPriceUSD, 1.2},
	...
	t.Run("Test file existence", func(t *testing.T) {
		...
		testConfigFile := LocalConfigFile{Filepath: "./testdata/Bogus.json"}
		assert.False(t, testConfigFile.Exists(), "This file should not exist")

		// Test existing file
		testConfigFile = LocalConfigFile{Filepath: "./testdata/Malformed.json"}
